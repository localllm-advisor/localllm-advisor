...
export default function Home() {
  ...
  useEffect(() => {
    // Handle ?buildFor=<modelId> param — navigate straight to hardware-finder mode
    const buildForParam = new URLSearchParams(window.location.search).get('buildFor');
    if (buildForParam) {
      setBuildForModelId(buildForParam);
      setMode('build-hardware');
      // Remove param from URL without reload so back-nav works cleanly
