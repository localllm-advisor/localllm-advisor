        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => { setShowLoginModal(false); setLoginError(null); }}>
          <div className={`relative w-full max-w-sm rounded-2xl border shadow-2xl p-6 ${isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'}`} onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              <button onClick={async () => { setLoginError(null); const res = await signInWithGitHub(); if (res?.error) setLoginError(res.error); else setShowLoginModal(false); }} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-800 hover:bg-gray-700 text-white rounded-lg font-medium transition-colors">
              ...
              <button onClick={async () => { setLoginError(null); const res = await signInWithGoogle(); if (res?.error) setLoginError(res.error); else setShowLoginModal(false); }} className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-colors border ${isDark ? 'bg-gray-800 hover:bg-gray-700 text-white border-gray-600' : 'bg-white hover:bg-gray-50 text-gray-700 border-gray-300'}`}>
