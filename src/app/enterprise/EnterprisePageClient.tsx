...
interface EnterprisePageClientProps {
  /** gpuCount + modelCount — read from JSON files at build time */
  totalHardwareAndModels: number;
}

export default function EnterprisePageClient({ totalHardwareAndModels }: EnterprisePageClientProps) {
      ...
      <main className="flex-1 w-full">
        ...
        <section className={`py-16 sm:py-24 border-b ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
          <div className="mx-auto max-w-4xl px-4 text-center">
            ...
            <Reveal delay={150}>
              {/* Stats — totalHardwareAndModels is computed at build time from dataset files */}
              <div className="flex flex-wrap justify-center gap-8 text-center">
                <div>
                  <p className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    <CountUp to={totalHardwareAndModels} suffix="+" />
                  </p>
