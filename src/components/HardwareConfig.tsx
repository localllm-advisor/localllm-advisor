...
export interface HardwareSpecs {
  ...
  memory_type?: 'GDDR6' | 'GDDR6X' | 'GDDR7' | 'HBM2' | 'HBM2e' | 'HBM3' | 'HBM3e' | 'Unified';
  cuda_cores?: number;
  ...
  // CPU
  cpu_name?: string;
  cpu_vendor?: 'intel' | 'amd' | 'apple' | 'qualcomm';
  cpu_cores?: number;
  cpu_threads?: number;
  p_cores?: number;
  e_cores?: number;
...
export default function HardwareConfig({
  ...
  // Auto-detect hardware on mount (async — WebGPU then WebGL fallback)
  useEffect(() => {
      ...
      {showAdvanced && (
        <div className={`rounded-xl border p-5 space-y-6 ${
          ...
          <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
            Override any spec manually. Empty fields use values from selected hardware.
          </p>

          {/* GPU Specs */}
          <div className="space-y-3">
            <h4 className={`text-xs font-semibold uppercase tracking-wide ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>GPU Specs</h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>VRAM (GB)</label>
                <input
                  type="number"
                  min="1"
                  max="256"
                  ...
                  placeholder="12"
                  className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 ${isDark ? 'border-gray-600 bg-gray-900/60 text-white placeholder-gray-600' : 'border-gray-300 bg-white text-gray-900 placeholder-gray-400'}`}
                />
              </div>
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Bandwidth (GB/s)</label>
                <input
                  type="number"
                  min="1"
                  max="5000"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Memory Type</label>
                <select
                  value={specs.memory_type ?? ''}
                  onChange={(e) => updateSpec('memory_type', e.target.value as HardwareSpecs['memory_type'] || undefined)}
                  className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 ${isDark ? 'border-gray-600 bg-gray-900/60 text-white' : 'border-gray-300 bg-white text-gray-900'}`}
                >
                  <option value="">-</option>
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Tensor Cores</label>
                <input
                  type="number"
                  min="0"
                  max="2000"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>CUDA Cores</label>
                <input
                  type="number"
                  min="0"
                  max="50000"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Stream Proc.</label>
                <input
                  type="number"
                  min="0"
                  max="50000"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>FP16 TFLOPS</label>
                <input
                  type="number"
                  min="1"
                  max="1000"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>FP32 TFLOPS</label>
                <input
                  type="number"
                  min="1"
                  max="500"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>INT8 TOPS</label>
                <input
                  type="number"
                  min="1"
                  max="2000"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>PCIe Gen</label>
                <select
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>PCIe Lanes</label>
                <select
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>TDP (W)</label>
                <input
                  type="number"
                  min="1"
                  max="1000"
          ...
          {/* Multi-GPU */}
          <div className="space-y-3">
            <h4 className={`text-xs font-semibold uppercase tracking-wide ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Multi-GPU</h4>
            <div className="flex items-end gap-4">
              <div className="w-32">
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>GPU Count</label>
                <input
                  type="number"
                  min="1"
                  max="16"
              ...
              <label className={`flex items-center gap-2 text-sm cursor-pointer pb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                <input
                  type="checkbox"
          ...
          {/* CPU Specs */}
          <div className="space-y-3">
            <h4 className={`text-xs font-semibold uppercase tracking-wide ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>CPU Specs</h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Cores</label>
                <input
                  type="number"
                  min="1"
                  max="256"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Threads</label>
                <input
                  type="number"
                  min="1"
                  max="512"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Base GHz</label>
                <input
                  type="number"
                  min="0.1"
                  max="10"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Boost GHz</label>
                <input
                  type="number"
                  min="0.1"
                  max="10"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>L3 Cache (MB)</label>
                <input
                  type="number"
                  min="1"
                  max="1024"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>P-Cores</label>
                <input
                  type="number"
                  min="0"
                  max="64"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>E-Cores</label>
                <input
                  type="number"
                  min="0"
                  max="64"
            ...
            <div className="flex flex-wrap gap-4 pt-1">
              {(['avx', 'avx2', 'avx512', 'amx'] as const).map((flag) => (
                <label key={flag} className={`flex items-center gap-2 text-sm cursor-pointer ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  <input
                    type="checkbox"
                    checked={(specs[flag] as boolean) ?? false}
                    onChange={(e) => updateSpec(flag, e.target.checked)}
                    className="rounded border-gray-400 text-blue-500 focus:ring-blue-500"
                  />
                  {flag === 'avx512' ? 'AVX-512' : flag.toUpperCase()}
                </label>
              ))}
            </div>
          </div>
          ...
          {/* System */}
          <div className="space-y-3">
            <h4 className={`text-xs font-semibold uppercase tracking-wide ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>System</h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>RAM (GB)</label>
                <input
                  type="number"
                  min="1"
                  max="4096"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>RAM Speed (MHz)</label>
                <input
                  type="number"
                  min="800"
                  max="10000"
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>RAM Channels</label>
                <select
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Storage Type</label>
                <select
              ...
              <div>
                <label className={`block text-xs mb-1 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Storage GB/s</label>
                <input
                  type="number"
                  min="0.1"
                  max="20"
          ...
          {/* Inference Preference */}
          <div className="space-y-3">
            <h4 className={`text-xs font-semibold uppercase tracking-wide ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Inference Preference</h4>
