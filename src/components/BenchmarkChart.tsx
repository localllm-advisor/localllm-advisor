          ...
          <div className="space-y-1">
            {(() => {
              const maxSpeed = Math.max(
                ...topModels.map((r) => r.performance.tokensPerSecondRange?.high ?? r.performance.tokensPerSecond ?? 0),
                1
              );
              return topModels.map((result, i) => {
                const range = result.performance.tokensPerSecondRange;
                const speed = result.performance.tokensPerSecond;
                const barWidth = range ? (range.estimate / maxSpeed) * 100 : speed !== null ? (speed / maxSpeed) * 100 : 0;
                return (
                  <div key={`speed-${result.model.id}`} className="flex items-center gap-2">
                    ...
                    <div className="w-20 text-xs text-gray-300 text-right">
                      {range ? `${range.low}–${range.high}` : speed !== null ? `${speed}` : '—'}
                    </div>
                  </div>
                );
              });
