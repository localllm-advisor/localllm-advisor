...
export default function ModelDetailModal({ model, onClose }: ModelDetailModalProps) {
  ...
  // Generate links
  const ollamaBase = (m.ollama_base ?? m.name ?? '').split(':')[0];
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard
              label="Speed"
              value={perf.tokensPerSecondRange
                ? `${perf.tokensPerSecondRange.low}–${perf.tokensPerSecondRange.high}`
                : '—'}
              suffix=" tok/s"
              isDark={isDark}
            />
          ...
          <Section title="Performance Estimates" isDark={isDark}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <InfoRow
                label="Decode Speed"
                value={perf.tokensPerSecondRange
                  ? `${perf.tokensPerSecondRange.low}–${perf.tokensPerSecondRange.high} tok/s`
                  : '—'}
                isDark={isDark}
              />
              <InfoRow
                label="Prefill Speed"
                value={perf.prefillRange
                  ? `${perf.prefillRange.low}–${perf.prefillRange.high} tok/s`
                  : '—'}
                isDark={isDark}
              />
              <InfoRow
                label="Time to First Token"
                value={perf.ttftRange
                  ? `${perf.ttftRange.low}–${perf.ttftRange.high}ms`
                  : '—'}
                isDark={isDark}
              />
              <InfoRow
