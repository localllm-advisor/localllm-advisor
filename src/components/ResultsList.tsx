          ...
          {selected && (
            <div className="mt-4 pt-3 border-t border-gray-700/50 flex flex-wrap items-center gap-2">
              ...
              {selected.performance.tokensPerSecond && (() => {
                const est = selected.performance.tokensPerSecond;
                const r = selected.performance.tokensPerSecondRange;
                return (
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-1 text-sm text-gray-400">
                  ...
                  {selected.performance.tokensPerSecondRange && ` \u00b7 ~${selected.performance.tokensPerSecondRange.low}\u2013${selected.performance.tokensPerSecondRange.high} tok/s`}
                </div>
              </div>
            ...
            <tbody className="divide-y divide-gray-800">
              ...
              <tr>
                ...
                {compareModels.map(r => {
                  ...
                  const range = r.performance.tokensPerSecondRange;
                  ...
                  return (
                    <td key={`speed-${r.model.id}`} className={`py-2 px-3 text-center ${isMax ? 'text-green-400' : 'text-white'}`}>
                      {range ? `${range.low}–${range.high} tok/s` : '—'}
                    </td>
                  );
                })}
              </tr>
              <tr>
                ...
                {compareModels.map(r => (
                  <td key={`prefill-${r.model.id}`} className="py-2 px-3 text-center text-white">
                    {r.performance.prefillRange
                      ? `${r.performance.prefillRange.low}–${r.performance.prefillRange.high} tok/s`
                      : '—'}
                  </td>
                ))}
              </tr>
              <tr>
                ...
                {compareModels.map(r => (
                  <td key={`ttft-${r.model.id}`} className="py-2 px-3 text-center text-white">
                    {r.performance.ttftRange
                      ? `${r.performance.ttftRange.low}–${r.performance.ttftRange.high}ms`
                      : '—'}
                  </td>
                ))}
              </tr>
...
function OurPickCard({
  ...
  const modelBenchmarks = model.model.benchmarks ?? {};
  const benchmarks = (USE_CASE_BENCHMARKS[useCase] ?? [])
    .filter(benchKey => modelBenchmarks[benchKey] !== null && modelBenchmarks[benchKey] !== undefined)
    .slice(0, 4);

  // Generate links
  const ollamaBase = (model.model.ollama_base ?? model.model.name ?? '').split(':')[0]; // Remove tag if present
  const ollamaLink = ollamaBase ? `https://ollama.com/library/${ollamaBase}` : '#';
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="lg:w-64 space-y-3">
          ...
          <div className="bg-gray-800/30 rounded-lg p-3">
            ...
            <div className="space-y-1.5">
              {benchmarks.map(benchKey => {
                const val = modelBenchmarks[benchKey];
                return (
